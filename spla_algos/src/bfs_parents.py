...
from spla_utils import copy_matrix, copy_vector
...
def bfs_parents(s: list[int], A: Matrix):
    ...
    zero_v = MAX_INT
    I, J, V = A.to_lists()  # noqa: E741
    A = Matrix.from_lists(I, J, V, shape=A.shape, dtype=A.dtype, zero_v=zero_v)
    s_values = list(map(lambda x: x + 1, s))
    n = A.n_rows
    n_indices = [i for i in range(n)]
    n_indices_values = [i + 1 for i in range(n)]
    s_indices = [i for i in range(len(s_values))]
    front: Matrix = Matrix.from_lists(
        s_indices,
        s,
        s_values,
        shape=(len(s_values), n),
        dtype=INT,
        zero_v=zero_v,
    )
    ...
    indices_matrix = Matrix.from_lists(
        ...
        [n_indices[i % n] for i in range(len(s) * n)],
        [n_indices_values[i % n] for i in range(len(s) * n)],
        shape=p.shape,
        dtype=p.dtype,
        zero_v=p._zero_V,
    )
    ...
    while found_count > 0:
        ...
        p.eadd(op_add=INT.S1ST_IF_SND_MAX_INT, M=p_masked, out=buffer)
        buffer.eadd(op_add=INT.S1ST_IF_SND_MAX_INT, M=p_masked, out=p)
